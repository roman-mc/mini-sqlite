...
const uint32_t INTERNAL_NODE_RIGHT_CHILD_OFFSET = INTERNAL_NODE_NUM_KEYS_OFFSET + INTERNAL_NODE_NUM_KEYS_OFFSET;
...
Table* db_open(const char* filename) {
    ...
    if (pager->num_pages == 0) {
        // new database file. Initialize page 0 as leaf
...
void leaf_node_split_and_insert(Cursor* cursor, uint32_t key, Row* value) {
    ...
    *leaf_node_next_leaf(old_node) = new_page_num;
    ...
    // all existing keys plus new key should be divided evenly between old (left_ and new (right) nodes.
    ...
    *leaf_node_num_cells(new_node) = LEAF_NODE_RIGHT_SPLIT_COUNT;
    ...
    // Then we need to update the nodes’ parent.
...
// binary search compares the key to find and the key to the right of the child pointer:
